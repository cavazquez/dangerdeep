...
void sky::smooth_and_equalize_bytemap(unsigned s, vector<Uint8> &map1) {
    ...
    for (unsigned y = 0; y < s; ++y) {
        ...
        for (unsigned x = 0; x < s; ++x) {
            ...
            unsigned v = (unsigned(map2[y1 * s + x1]) + unsigned(map2[y1 * s + x2]) + unsigned(map2[y2 * s + x1]) + unsigned(map2[y2 * s + x2])) / 16 + (unsigned(map2[y * s + x1]) + unsigned(map2[y * s + x2]) + unsigned(map2[y1 * s + x]) + unsigned(map2[y2 * s + x])) / 8 + (unsigned(map2[y * s + x])) / 4;
            map1[y * s + x] = Uint8(v);
            if (v < minv)
                minv = v;
            if (v > maxv)
