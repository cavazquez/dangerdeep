...
#define SDL_SRCCOLORKEY 0x00001000 // !!Rake: tempory fix till the
...
void texture::init(const vector<Uint8> &data, bool makenormalmap, float detailh) {
    ...
    if (makenormalmap && format == GL_LUMINANCE) {
        if (dimension != GL_TEXTURE_2D)
            throw texerror(get_name(), "normals only supported for 2D textures");
        ...
        if (do_mipmapping[mapping]) {
            if (dimension != GL_TEXTURE_2D)
                throw texerror(get_name(), "mip mapping only supported for 2D textures");
#if 1
...
void texture::draw_tiles(int x, int y, int w, int h, const colorf &col) const {
    ...
    primitives::textured_quad(vector2f(x, y), vector2f(x + w, y + h), *this,
                              vector2f(0, 0), vector2f(tilesx, tilesy), col)
        .render();
}
...
void texture::draw_subimage(int x, int y, int w, int h, unsigned tx, unsigned ty,
    ...
    primitives::textured_quad(vector2f(x, y), vector2f(x + w, y + h), *this,
                              vector2f(x1, y1), vector2f(x2, y2), col)
        .render();
}
