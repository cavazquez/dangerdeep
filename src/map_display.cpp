...
void map_display::draw_visual_contacts(class game &gm,
    ...
    for (vector<sea_object *>::const_iterator it = objs.begin(); it != objs.end(); ++it) {
        color c;
        if (dynamic_cast<const submarine *>(*it))
            c = color(255, 255, 128);
        else if (dynamic_cast<const torpedo *>(*it))
            c = color(255, 0, 0);
        else if (dynamic_cast<const ship *>(*it))
            c = color(192, 255, 192);
        else if (dynamic_cast<const airplane *>(*it))
            c = color(0, 0, 64);
...
map_display::map_display(user_interface &ui_) : user_display(ui_), mapzoom(0.1), mx(0), my(0), mapmode(0),
                                                edit_btn_del(0),
                                                edit_btn_chgmot(0),
                                                edit_btn_copy(0),
                                                edit_btn_cvmenu(0),
    ...
    if (gm.is_editor()) {
        ...
        if (event.type == SDL_EVENT_KEY_DOWN) {
            if (event.key.key == SDLK_LSHIFT) {
                ...
                ctrl_key_pressed |= 1;
                return;
            } else if (event.key.key == SDLK_RCTRL) {
                ctrl_key_pressed |= 2;
                return;
            }
