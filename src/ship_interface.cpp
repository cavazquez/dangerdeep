...
bool ship_interface::keyboard_common(int keycode, class game &gm) {
    ...
    case SDLK_RETURN:
        ...
        break;
    case SDLK_1:
        gm.send(new command_set_throttle(player, sea_object::aheadslow));
        add_message(texts::get(43));
        break;
    case SDLK_2:
        gm.send(new command_set_throttle(player, sea_object::aheadhalf));
        add_message(texts::get(44));
        break;
    case SDLK_3:
        gm.send(new command_set_throttle(player, sea_object::aheadfull));
        add_message(texts::get(45));
        break;
    case SDLK_4:
        gm.send(new command_set_throttle(player, sea_object::aheadflank));
        add_message(texts::get(46));
        break; // flank/full change? fixme
    case SDLK_5:
        gm.send(new command_set_throttle(player, sea_object::stop));
        add_message(texts::get(47));
        break;
    case SDLK_6:
        gm.send(new command_set_throttle(player, sea_object::reverse));
        add_message(texts::get(48));
        break;

    // view
    case SDLK_COMMA:
        bearing -= angle(sys().key_shift() ? 10 : 1);
        break;
    case SDLK_PERIOD:
        bearing += angle(sys().key_shift() ? 10 : 1);
        break;
    ...
    case SDLK_SPACE:
        target = gm.sub_in_direction_from_pos(player, player->get_heading() + bearing);
        if (!target.is_null())
...
void ship_interface::draw_gauge(unsigned nr, int x, int y,
    ...
    switch (nr) {
    case 1:
        gauge1->draw(x, y, wh, wh);
        break;
    case 2:
        gauge2->draw(x, y, wh, wh);
        break;
    case 3:
        gauge3->draw(x, y, wh, wh);
        break;
    case 4:
        gauge4->draw(x, y, wh, wh);
        break;
    default:
        return;
    }
