...
system::parameters::parameters() : near_z(1.0),
                                   far_z(1000.0),
                                   resolution_x(1680),
                                   resolution_y(1050),
                                   fullscreen(false),
...
system::parameters::parameters(double near_z_, double far_z_, unsigned res_x, unsigned res_y, bool fullscreen_) : near_z(near_z_),
                                                                                                                  far_z(far_z_),
                                                                                                                  resolution_x(res_x),
                                                                                                                  resolution_y(res_y),
...
int system::transform_2d_x(int x) {
    ...
    if (x < 0)
        x = 0;
    if (x >= int(res_area_2d_w))
        x = int(res_area_2d_w) - 1;
...
int system::transform_2d_y(int y) {
    ...
    if (y < 0)
        y = 0;
    if (y >= int(res_area_2d_h))
        y = int(res_area_2d_h) - 1;
    return y * res_y_2d / res_area_2d_h;
}

void system::prepare_2d_drawing() {
    if (draw_2d)
        throw runtime_error("2d drawing already turned on");
    glFlush();
