...
std::unique_ptr<widget> widget::create_dialogue_ok(widget *parent_, const string &title, const string &text,
    ...
    if (!h)
        h = res_y / 2;
    auto wi = std::make_unique<widget>(x, y, w, h, title, parent_);
...
void widget_list::draw() const {
    ...
    for (unsigned lp = 0; it != entries.end() && lp < maxp; ++it, ++lp) {
        color tcol = !is_enabled() ? globaltheme->textdisabledcol : (selected == int(lp + listpos)) ? globaltheme->textselectcol
                                                                                                    : globaltheme->textcol;
...
void widget_list::on_click(int mx, int my, int mb) {
    ...
    if (mb & 1) {
        ...
        if (unsigned(selected) >= entries.size())
