...
ships_sunk_display::ships_sunk_display(user_interface &ui_) : user_display(ui_), first_displayed_object(0) {
}
...
void ships_sunk_display::display(class game &gm) const {
    ...
    for (unsigned i = 0; it != sunken_ships.end() && i < 12; ++it, ++i) {
        ...
        if (mdl) {
            ...
            log_warning("can't find model for that name, BUG?!" << data_file().get_rel_path(it->specfilename) << it->mdlname);
        }
        glPopMatrix();
    }
