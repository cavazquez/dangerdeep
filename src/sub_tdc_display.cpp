...
void sub_tdc_display::process_input(class game &gm, const SDL_Event &event) {
    ...
    if (show_screen1) {
        if (!myscheme1.get())
            throw error("sub_tdc_display::process_input without scheme!");
        ...
        case SDL_EVENT_MOUSE_BUTTON_DOWN: {
            ...
            if (mx >= s.parallax_ptr.centerx - parasz && mx <= s.parallax_ptr.centerx + parasz && my >= s.parallax_ptr.centery - parasz && my <= s.parallax_ptr.centery + parasz) {
